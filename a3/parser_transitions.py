...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        mini_batch = unfinished_parses[:cur_size]
        next_trans = model.predict(mini_batch)
        
        for pp,t in zip(mini_batch,next_trans):
            # print("#1:pp.stack: {:} pp.buffer: {:} next_tran: {:}".format(pp.stack,pp.buffer,t))
            pp.parse_step(t)
            # print("#2:pp.stack: {:} pp.buffer: {:} ".format(pp.stack,pp.buffer))
